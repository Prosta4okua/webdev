...
//зверніть увагу на можливість використання ``,яка дозволяє додавати в string змінні ${}
           ...
           <a class='link' href='${slide.link}' target='_blank'>Open</a></div></div>`;
            ...
            btn.addEventListener("click", ()=> {
                    ...
                    // while (good_index !== this.current)
                    console.log(indexx !== slide.id.slice(-1))
                    // if (index !== slide.id.slice(-1))
                    //     this.prevSlide();
                    console.log("start");
            ...
            // document.getElementById(btn.id).addEventListener("click", this.setSlide(good_index));
            // btn.addEventListener("click", this.setSlide(good_index))
            // myDiv.appendChild(btn);
            good_index++;
        }
